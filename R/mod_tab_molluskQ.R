...
#'
#' @examples
mod_molluskQ_ui <- function(id, tabname) {
    ...
    fluidRow(
      box(
        ...
        textInput(inputId = ns("title"), label = "Title",
                  value = "Quantification of Mollusks"),
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      molluskQ <- reactive({
        ...
        molluskQ <- get_resources(resource_category = "QuantMollusk") %>%
          remove_na_cols() %>%
      ...
      generateLayerSelector("layers", molluskQ, inputid = ns("selected_layers"))

      make_plot <- reactive({

        validate(
          need(is.data.frame(molluskQ()), "Waiting for data...")
        )

        existing_cols <- colnames(molluskQ())
        keep <- existing_cols
        keep <- keep[grepl(input$plot_by, keep, ignore.case = TRUE)]
        # remove "countTotal" as well
        keep <- keep[!grepl(paste0(input$plot_by, "Total", ignore.case = TRUE), keep)]
        # needed for melt id
